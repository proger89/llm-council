...
function MessageBubble({ message, isExpanded, onToggleExpand }) {
        ...
        <div className={`flex-1 overflow-hidden ${isUser ? 'text-right' : ''}`}>
          ...
          {hasDiscussion && (
            <button
              ...
              className="mt-2 flex items-center gap-1 text-sm text-council-accent hover:text-council-accent/80 transition-colors"
            >
              {isExpanded ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
              <span>
                {isExpanded ? 'Скрыть' : 'Показать'} обсуждение совета
              </span>
            </button>
          )}
        </div>
